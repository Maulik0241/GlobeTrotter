    ...
    <div className="space-y-8 pb-16">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-400">Manage, organize, and customize your travel plans</p>
        </div>

        <button
          onClick={openCreateTripModal}
          className="flex items-center gap-2 px-5 py-3 rounded-2xl bg-gradient-to-r from-teal-500 to-cyan-500 hover:from-teal-400 hover:to-cyan-400 text-slate-950 font-bold text-sm transition-all shadow-xl shadow-teal-500/25 active:scale-95 self-start sm:self-auto"
        >
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-slate-900 border border-slate-800 p-4 rounded-3xl shadow-lg">
        <div className="relative w-full sm:w-80">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2 bg-slate-950 border border-slate-800 rounded-2xl text-slate-100 placeholder-slate-500 text-xs focus:outline-none focus:border-teal-500"
          />
        </div>

        <div className="flex items-center gap-2 bg-slate-950 p-1 rounded-2xl border border-slate-800 self-end sm:self-auto">
          <button
            onClick={() => setViewMode('grid')}
            className={`p-2 rounded-xl text-xs font-semibold flex items-center gap-1.5 transition-colors ${
              viewMode === 'grid' ? 'bg-teal-500 text-slate-950 font-bold' : 'text-slate-400 hover:text-slate-200'
            }`}
          >
          ...
          <button
            ...
            className={`p-2 rounded-xl text-xs font-semibold flex items-center gap-1.5 transition-colors ${
              viewMode === 'table' ? 'bg-teal-500 text-slate-950 font-bold' : 'text-slate-400 hover:text-slate-200'
            }`}
          >
              ...
              >
                <div className="relative h-48 overflow-hidden">
                  ...
                  <div className="absolute top-3 left-3 flex items-center gap-2">
                    <span className="px-2.5 py-1 rounded-full text-xs font-bold bg-slate-950/80 backdrop-blur-md text-teal-300 border border-slate-700">
                  ...
                  <button
                    ...
                    className="absolute top-3 right-3 p-2 rounded-full bg-slate-950/70 hover:bg-rose-500 text-slate-400 hover:text-white transition-colors"
                    title="Delete Trip"
                  >
                ...
                <div className="p-5 space-y-4 flex-1 flex flex-col justify-between">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {trip.stops.map((stop) => (
                      <span
                        key={stop.id}
                        className="px-2.5 py-1 rounded-lg text-xs font-medium bg-slate-950 text-slate-300 border border-slate-800 flex items-center gap-1"
                      >
                  ...
                  <div className="p-3 bg-slate-950/70 rounded-2xl border border-slate-800/80 flex items-center justify-between text-xs">
                    ...
                    <div className="text-right">
                      <span className="text-slate-500 text-[10px] block">Share Code</span>
                  ...
                  <div className="grid grid-cols-3 gap-2 pt-2 border-t border-slate-800">
                    ...
                    <button
                      ...
                      className="py-2.5 rounded-xl bg-teal-500/10 hover:bg-teal-500/20 text-teal-300 border border-teal-500/20 text-xs font-semibold flex items-center justify-center gap-1 transition-colors"
                    >
                  ...
                  return (
                    <tr key={trip.id} className="hover:bg-slate-800/40 transition-colors">
                      <td className="px-6 py-4 font-bold text-slate-100 flex items-center gap-3">
                        <img src={trip.cover_photo} alt={trip.name} className="w-10 h-10 rounded-xl object-cover" />
                        <div>
                          <span>{trip.name}</span>
                      ...
                      <td className="px-6 py-4">
                        <span className="px-2.5 py-1 rounded-full bg-slate-950 text-teal-300 border border-slate-800 font-semibold">
                          {trip.stops.length} Cities
                        </span>
                      </td>
                      ...
                      <td className="px-6 py-4 text-right space-x-2">
                        ...
                        <button
                          ...
                          className="px-3 py-1.5 rounded-lg bg-teal-500/20 text-teal-300 hover:bg-teal-500/30 font-semibold"
                        >
                          Builder
                        </button>
                      </td>
